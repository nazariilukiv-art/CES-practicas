...
public class UsuarioDAOImp implements UsuarioDAO {
    ...
    public int borrarUsuarios(String nombre) {
        // 1. Preparamos la consulta SQL usando tus constantes de SchemeDB
        String query = String.format("DELETE FROM %s WHERE %s = ?",
                SchemeDB.TAB_NAME, SchemeDB.COL_NAME);

        try {
            // 2. Creamos el PreparedStatement y asignamos el nombre
            preparedStatement = connection.prepareStatement(query);
            preparedStatement.setString(1, nombre);

            // 3. Ejecutamos el borrado y devolvemos las filas afectadas
